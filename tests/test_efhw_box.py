...
class BoxModelTests:
    ...
    def _text(self):
        return self.scad.read_text(encoding="utf-8")

    def test_scad_exists(self):
        self.assertTrue(self.scad.is_file(), f"missing {self.scad}")
    ...
    def test_trough_and_boss_params(self):
        ...
        self.assertIn("circle(d = 0.02)", bosses)
    ...
    def test_so239_and_m4_keywords(self):
        ...
        m4 = text[text.index("module m4_cutout") : text.index("module m4_plug")]
        plug = text[text.index("module m4_plug") : text.index("module lid_label")]
        ...
        self.assertIn("for (sy = [-1, 1])", pad)
        self.assertIn("for (sy = [-1, 1])", m4)
    ...
    def test_label_uses_ratio_and_power_line(self):
        ...
        self.assertIn(self.power_label, lab)
    ...
    def test_openscad_csg_base(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            r = self._openscad(
                ["-o", str(out), "-D", 'part="base"', str(self.scad)]
            )
    ...
    def test_openscad_stl_base_and_lid(self):
        with tempfile.TemporaryDirectory() as td:
            for p, name in [("base", "base.stl"), ("lid", "lid.stl")]:
                out = Path(td) / name
                r = self._openscad(
                    ["-o", str(out), "-D", f'part="{p}"', str(self.scad)]
                )
                self.assertEqual(r.returncode, 0, r.stderr)
...
class RhombBoxTests:
    def test_rhombic_two_hole_so239(self):
        text = self._text()
        self.assertRegex(text, r"so_hole_spacing\s*=\s*25")
        so = text[text.index("module so239_cutout") : text.index("module m4_pad")]
        self.assertNotIn("for (a = [-1, 1], b = [-1, 1])", so)
        self.assertIn("so239_rhomb_profile", text)


class TestEfhwBox(BoxModelTests, unittest.TestCase):
    scad = ROOT / "efhw_box.scad"
    inner_z = 38
    power_label = "250W SSB PEP"
    ...
    def test_readme_lists_all_models(self):
        text = README.read_text(encoding="utf-8")
        self.assertIn("efhw_box.scad", text)
        self.assertIn("efhw_box_2x.scad", text)
        self.assertIn("efhw_box_rhomb.scad", text)
        self.assertIn("efhw_box_2x_rhomb.scad", text)
        self.assertIn("2 × FT 240-43", text)
        self.assertIn("ромбическ", text)


class TestEfhwBox2x(BoxModelTests, unittest.TestCase):
    scad = ROOT / "efhw_box_2x.scad"
    inner_z = 50
    power_label = "500W SSB PEP"


class TestEfhwBoxRhomb(BoxModelTests, RhombBoxTests, unittest.TestCase):
    scad = ROOT / "efhw_box_rhomb.scad"
    inner_z = 38
    power_label = "250W SSB PEP"


class TestEfhwBox2xRhomb(BoxModelTests, RhombBoxTests, unittest.TestCase):
